...
const apiUrlProducts = process.env.API_URL_PRODUCTS;
...
exports.getProducts = (searchValue, res) => {
    axios.get(
            `${apiUrl}${apiUrlProducts}search?q=${searchValue}&limit=${productLimit}`)
        .then((response) => {
 ...
 */
const getAuthor = () => ({ name: 'Gabriel', lastname: 'Solarte' });
